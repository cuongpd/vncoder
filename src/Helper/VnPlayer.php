...
class VnPlayer
{
    public static function getVideoID($link)
    {
        ...
        if (!empty($link)) {
            if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $link, $match)) {
                ...
                $videoID   = $match[1];
                $videoType = 'youtube';
            } elseif (preg_match('/(?:https?:\/\/)?(?:[\w\-]+\.)*(?:drive|docs)\.google\.com\/(?:(?:folderview|open|uc)\?(?:[\w\-\%]+=[\w\-\%]*&)*id=|(?:folder|file|document|presentation)\/d\/|spreadsheet\/ccc\?(?:[\w\-\%]+=[\w\-\%]*&)*key=)([\w\-]{28,})/i', $link, $match)) {
                $isVideo   = true;
                $videoID   = $match[1];
                $videoType = 'google';
            } elseif (preg_match("#https?://vimeo.com/([0-9]+)#i", $link, $match)) {
                $isVideo   = true;
                $videoID   = $match[1];
                $videoType = 'vimeo';
            } elseif (preg_match('#https?:.*?\.(mp4|mov)#s', $link, $match)) {
                $isVideo   = true;
                $videoType = 'mp4';
                $videoID   = $match[0];
            } elseif (preg_match('#https://www.dailymotion.com/video/([A-Za-z0-9]+)#s', $link, $match)) {
                $videoID   = $match[1];
                $videoType = 'daily';
                $isVideo   = true;
            } elseif (preg_match('#(https://www.ok.ru/|https://ok.ru/)(video|live)/([A-Za-z0-9]+)#s', $link, $match)) {
                $videoID   = $match[3];
                $videoType = 'ok';
                $isVideo   = true;
            } elseif (preg_match('@^(?:https?:\/\/)?(?:www\.|go\.)?twitch\.tv(\/videos\/([A-Za-z0-9]+)|\/([A-Za-z0-9]+)\/clip\/([A-Za-z0-9]+)|\/(.*))($|\?)@', $link, $match)) {
            ...
            } elseif (preg_match('~([A-Za-z0-9]+)/videos/(?:t\.\d+/)?(\d+)~i', $link, $match)) {
                $videoID   = $match[0];
                $videoType = 'facebook';
                $isVideo   = true;
            }
    ...
    public static function getVideoPlayer($link)
    {
        $videoInfo = self::getVideoID($link);
        $data=[];
        ...
        if (!$isVideo) {
            ...
            switch ($videoType) {
                case "youtube":
                    $data["ok"]='<iframe class="vnTube" id="vnTube" src="https://www.youtube.com/embed/'.$videoID.'?enablejsapi=1&controls=1&fs=0&iv_load_policy=3&rel=0&showinfo=0&autoplay=1&rel=0&modestbranding=1&autohide=1" width="100%" height="100%" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
                    break;
